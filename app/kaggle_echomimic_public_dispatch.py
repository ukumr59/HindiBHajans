"""EchoMimicV3 Kaggle dispatcher that never calls kernels.get.

The existing EchoMimic worker is reused, but the control plane uses only
Kaggle kernel push plus the public output-download HTTP endpoint. This avoids
the current 403 Permission 'kernels.get' failure in the Kaggle CLI.
"""
...
def main():
    ...
    with zipfile.ZipFile(io.BytesIO(data)) as z: z.extractall(outdir)
    xs=list(outdir.rglob('master.mp4'))
    if not xs: raise RuntimeError('KAGGLE_ECHOMIMIC_MASTER_MISSING: public output downloaded but master.mp4 was absent')
