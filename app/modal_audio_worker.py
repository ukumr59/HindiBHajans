...
def generate(seconds: int = 180) -> bytes:
    ...
    subprocess.run(["git","clone","--depth","1","https://github.com/ACE-Step/ACE-Step-1.5.git",str(repo)],check=True)
    subprocess.run(["python","-m","pip","install","-q","-e","."],cwd=repo,check=True)
    ...
    print("MODAL_AUDIO_GPU",torch.cuda.get_device_name(0),flush=True)
    dit=AceStepHandler(); dit.initialize_service(project_root=str(repo),config_path="acestep-v15-turbo",device="cuda",offload_to_cpu=True)
    llm=LLMHandler(); llm.initialize(checkpoint_dir=str(repo/"checkpoints"),lm_model_path="acestep-5Hz-lm-0.6B",backend="pt",device="cuda")
    ...
    subprocess.run(["ffmpeg","-y","-v","error","-i",str(source),"-af","loudnorm=I=-9:TP=-1.0:LRA=7","-ar","48000","-ac","2","-c:a","libmp3lame","-b:a","320k",str(out)],check=True)
