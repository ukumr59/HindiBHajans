...
def generate(image_bytes:bytes,audio_bytes:bytes,seconds:int=180)->bytes:
 ...
 # The upstream Flash script calls pipeline.to(cuda), which places all components on a 16-GB T4 at once.
 # Patch that exact upstream operation to Diffusers sequential CPU offload before executing inference.
